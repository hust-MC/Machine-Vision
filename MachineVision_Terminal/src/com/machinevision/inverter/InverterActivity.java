...
public class InverterActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mTextProm = (TextView)findViewById(R.id.text_pro);
		...
		public void handleMessage(Message msg)
		{
			if(sciModel.isSciOpened()) {
				...
				case SciModel.Data_ACK:
					EToast.makeText(getApplicationContext(), "操作正确", EToast.LENGTH_SHORT).show();
				...
				case SciModel.Data_NAK:
					...
					EToast.makeText(getApplicationContext(), "数据错误为：" + daErr, EToast.LENGTH_LONG).show();
					mLayoutMain.setClickable(true);
					break;
				...
				case SciModel.Conn_Error:
					if(++time == 10) {
						time = 0;				
						EToast.makeText(getApplicationContext(), "连接超时,检查连线", EToast.LENGTH_SHORT).show();
					}
					mLayoutMain.setClickable(true);
					break;
				case SciModel.Get_Alarm:
					int aCode = msg.arg1;
					EToast.makeText(getApplicationContext(), RecvUtils.alarms_title[aCode], EToast.LENGTH_LONG).show();
					break;
				}
				mLayoutMain.setClickable(true);
			}
		...
		if(sciModel.isSciOpened()) {
			...
			case R.id.button_setfrq_ram:
				if(TextUtils.isEmpty(mEdtextRam.getText()))
					EToast.makeText(getApplicationContext(), "请输入设置频率", EToast.LENGTH_SHORT).show();
				else {
					int frqRam = Integer.parseInt(mEdtextRam.getText().toString());
					if(frqRam > 50)
						EToast.makeText(getApplicationContext(), "超过限制频率", EToast.LENGTH_SHORT).show();
					else {
						mEdtextRam.setText("");
...
//			mLayoutMain.setClickable(false);
		...
		else EToast.makeText(this, "请先开启串口", EToast.LENGTH_SHORT).show();
	}
	
	private void showAlertDialog() {
		ContextThemeWrapper contextThemeWrapper = 
                new ContextThemeWrapper(InverterActivity.this, R.style.dialog);
