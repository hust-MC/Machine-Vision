...
public class InverterActivity extends Activity
{
		...
		public void handleMessage(Message msg)
		{
			if (sciModel.isSciOpened())
			{
				...
				case SciModel.Data_ACK:
					Toast.makeText(getApplicationContext(), "操作正确",
				...
				case SciModel.Data_NAK:
					...
					Toast.makeText(getApplicationContext(), "数据错误为：" + daErr,
				...
				case SciModel.Conn_Error:
					...
					{
						time = 0;
						Toast.makeText(getApplicationContext(), "连接超时,检查连线",
								Toast.LENGTH_SHORT).show();
					}
					mLayoutMain.setClickable(true);
					break;
				case SciModel.Get_Alarm:
					...
					Toast.makeText(getApplicationContext(),
							RecvUtils.alarms_title[aCode], Toast.LENGTH_LONG)
							.show();
					break;
				}
			}
		...
		if (sciModel.isSciOpened())
		{
			...
			case R.id.button_setfrq_ram:
				if (TextUtils.isEmpty(mEdtextRam.getText()))
					Toast.makeText(getApplicationContext(), "请输入设置频率",
							Toast.LENGTH_SHORT).show();
				else
				{
					int frqRam = Integer.parseInt(mEdtextRam.getText()
							.toString());
					if (frqRam > 50)
						Toast.makeText(getApplicationContext(), "超过限制频率",
								Toast.LENGTH_SHORT).show();
					else
	...
	public void onSciClick(View view)
	{
		...
		// case R.id.Sci_init:
		// if(sciModel.isSciOpened())
		// sciModel.sThreadOpen();
		// break;
		// case R.id.Sci_close:
		// sciModel.setSTOpened(false);
		// break;
		case R.id.btn_back:
			onBackPressed();
			break;
