...
import com.machinevision.svm_src.*;
...
class svm_predict {
	...
	public static Double main(String argv[]) throws IOException
	{
		...
		try 
		{
			File file = new File(FILE_DIR + argv[i]);
			BufferedReader input = new BufferedReader(new FileReader(file));
			
			DataOutputStream output = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(FILE_DIR + argv[i+2])));
			svm_model model = svm.svm_load_model(FILE_DIR, argv[i+1]);
			if(predict_probability == 1)
			{
